...
function _availableOperatorsFields(fieldName, itc, useOperators) {
    ...
    operators.forEach((operatorName) => {
        const fieldType = graphql_1.getNamedType(itc.getFieldType(fieldName));
        if (fieldType) {
            if (['in', 'nin', 'in[]', 'nin[]'].includes(operatorName)) {
                ...
                if (operatorName === 'exists') {
                    ...
                    if (fieldType === graphql_1.GraphQLString) {
                        fields[operatorName] = { type: RegExpAsString_1.default };
                    }
                }
                else {
                    fields[operatorName] = { type: fieldType };
                }
            }
        }
    });
...
function _recurseSchema(inputITC, sourceITC, opts, indexedFields, pathName) {
    ...
    sourceITC.getFieldNames().forEach((fieldName) => {
        ...
        if (sourceITC.getType() === fieldType)
            return;
        ...
        if (fieldType instanceof graphql_1.GraphQLScalarType || fieldType instanceof graphql_1.GraphQLEnumType) {
        ...
        else if (fieldType instanceof graphql_1.GraphQLInputObjectType) {
